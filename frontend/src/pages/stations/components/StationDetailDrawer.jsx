  ...
  const columns = [
    {
      key: 'pollutant',
      title: '监测因子',
      render: (row) => row.pollutant_label || row.pollutant
    },
    {
      key: 'unit',
      title: '单位',
      render: (row) => <span className="muted small">{row.unit}</span>
    },
    ...
    { key: 'exceeded_count', title: '超标', align: 'right', render: (row) => (row.exceeded_count ? <span className="danger-text">{row.exceeded_count}</span> : '0') },
    {
      ...
      title: '均值',
      align: 'right',
      render: (row) => (row.avg_value === null || row.avg_value === undefined ? '-' : formatFixed(row.avg_value, row.precision ?? 2))
    },
    {
      key: 'max_value',
      title: '最大值',
      align: 'right',
      render: (row) => (row.max_value === null || row.max_value === undefined ? '-' : formatFixed(row.max_value, row.precision ?? 2))
    }
  ]
